...
export const getMyPostsService = async (member_id: number) => {
	const [ posts, totalPosts, totalLikes, totalComments ] = await Promise.all([
...
export const createPostService = async (memberId: number, content: string, files: Express.Multer.File[]) => {
...
export const toggleLikeService = async (member_id: number, post_id: number) => {
...
export const createCommentService = async (post_id:number, member_id:number, comment:string)=>{
